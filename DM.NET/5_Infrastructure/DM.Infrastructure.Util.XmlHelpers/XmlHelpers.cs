...
namespace DM.Infrastructure.Util.XmlHelpers
{
    /// <summary>
    ...
    public static class XmlHelpers
    {
        ...
        /// <summary>
        /// Get a XSL compiled transform from the cache, add to cache if necessary.
        /// If the XSL file changes, it will be automatically refreshed in the cache.
        /// </summary>
        /// <param name="xslFileName">Full path to XSL file</param>
        /// <returns>XSL compiled transform for given XSL file</returns>
        private static XslCompiledTransform GetTransform(string xslFileName)
        {
            XslCompiledTransform xsl = xslCache[xslFileName] as XslCompiledTransform;
            if (xsl == null)
            {
                xsl = new XslCompiledTransform(ConfigurationManager.AppSettings["XslDebug"] == "1");
                xsl.Load(xslFileName, new XsltSettings(true, true), new XmlUrlResolver());

                // add compiled transform to cache with a dependency on the xsl file
                // if the xsl file changes, it will be automatically reloaded
                xslCache.Add(xslFileName, xsl, CacheItemPriority.Normal, null, new FileDependency(xslFileName));
            }
        ...
        /// <summary>
        /// Perform an xml transformation on a reader.
        /// </summary>
        /// <param name="xslFileName">Full path of xsl file to use for transformation.</param>
        /// <param name="reader">Xml document to transform.</param>
        /// <returns>Transformed xml.</returns>
        public static XmlReader Transform(string xslFileName, XmlReader reader)
        {
            return XmlHelpers.Transform(xslFileName, reader, null);
        }
        ...
        /// <param name="xslFileName">Full path of xsl file to use for transformation.</param>
        /// <param name="reader">Xml document to transform.</param>
        /// <param name="parameters">Xsl transformation argument list, may be null if no parameters.</param>
        /// <returns>Transformed xml.</returns>
        public static XmlReader Transform(string xslFileName, XmlReader reader, XsltArgumentList parameters)
        {
            MemoryStream stream = new MemoryStream();

            try
            {
                XslCompiledTransform xsl = GetTransform(xslFileName);
                xsl.Transform(reader, parameters, stream);

                //flush and reset memory stream for reading
                stream.Flush();
                stream.Position = 0;

                //note that XmlReader.Close will close underlying MemoryStream
                XmlReaderSettings xrs = new XmlReaderSettings();
                xrs.ConformanceLevel = ConformanceLevel.Fragment;

                return XmlReader.Create(stream, xrs);
            }
            catch (Exception)
            {
                stream.Close();
        ...
        /// <summary>
        /// Perform an xsl transformation on document in memory.
        /// </summary>
        /// <param name="xslFileName">Full pathname of xsl file to use for transformation.</param>
        /// <param name="doc">Xml document to transform.</param>
        /// <returns>Transformed xml.</returns>
        public static XmlReader Transform(string xslFileName, IXPathNavigable doc)
        {
            return XmlHelpers.Transform(xslFileName, doc, null);
        }
        ...
        /// <param name="parameters">Xsl transformation argument list, may be null if no parameters.</param>
        /// <returns>Transformed xml.</returns>
        public static XmlReader Transform(string xslFileName, IXPathNavigable doc, XsltArgumentList parameters)
        {
            MemoryStream stream = new MemoryStream();
            ...
            {
                XslCompiledTransform xsl = GetTransform(xslFileName);
                xsl.Transform(doc, parameters, stream);
